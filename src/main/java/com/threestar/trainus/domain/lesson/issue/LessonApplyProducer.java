...
public class LessonApplyProducer {
	...
	public String send(Long lessonId, Long userId) {
		...
		Long addedCount = coreRedisTemplate.opsForSet().add(duplicateKey, String.valueOf(userId));
		...
		// 최초 신청 시에만 TTL 설정
		coreRedisTemplate.expire(duplicateKey, Duration.ofMinutes(LessonApplyStreamConstant.DUPLICATE_TTL_MINUTE));

		// Redis 원자 연산으로 재고 차감
		Long stock = coreRedisTemplate.opsForValue().decrement(stockKey);

		// 예외 및 재고 소진 처리
		if (stock == null) {
			coreRedisTemplate.opsForSet().remove(duplicateKey, String.valueOf(userId));
			Metrics.counter("lesson_apply_total", "version", "mq", "result", "system_error").increment();
			return null;
		}
		...
		if (stock < 0) {
			...
			coreRedisTemplate.opsForValue().increment(stockKey);
			coreRedisTemplate.opsForSet().remove(duplicateKey, String.valueOf(userId));
			return null;
		}

		// requestId 생성 및 대기열 등록
		String requestId = UUID.randomUUID().toString();

		// 원자적 순번 기반으로 대기열 등록
		waitingRoomService.enqueue(requestId, lessonId, userId, stock);
	...
	public void setStock(Long lessonId, int stock) {
		String stockKey = LessonApplyStreamConstant.STOCK_PREFIX + lessonId;
		coreRedisTemplate.opsForValue().set(stockKey, String.valueOf(stock));
